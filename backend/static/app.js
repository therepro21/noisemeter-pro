...
Object.assign(STATIC_EN, {'Schallpegelüberwachung · Version 3.1':'Sound level monitoring · Version 3.1','Software-Update':'Software update','Update installieren':'Install update','Messwerte als Retain veröffentlichen':'Publish measurements as retained','TLS aktivieren':'Enable TLS','CA-Zertifikatspfad (optional)':'CA certificate path (optional)','Update-Status wird geprüft …':'Checking update status …'});
...
async function loadConfig() {
  ...
  $('#mqtt-enabled').checked = value.mqtt.enabled; $('#mqtt-host').value = value.mqtt.host; $('#mqtt-port').value = value.mqtt.port; $('#mqtt-client-id').value = value.mqtt.client_id || 'noisemeter-pro'; $('#mqtt-user').value = value.mqtt.username; $('#mqtt-keepalive').value = value.mqtt.keepalive || 60; $('#mqtt-qos').value = value.mqtt.qos ?? 0; $('#mqtt-retain').checked = value.mqtt.retain !== false; $('#mqtt-tls').checked = !!value.mqtt.tls_enabled; $('#mqtt-ca-file').value = value.mqtt.tls_ca_file || ''; $('#mqtt-discovery').value = value.mqtt.discovery_prefix; $('#mqtt-topic').value = value.mqtt.base_topic; $('#mqtt-status').textContent = value.mqtt.connected ? ui('● Mit Broker verbunden','● Connected to broker') : ui('Nicht mit Broker verbunden','Not connected to broker');
  return value;
}
...
$('#open-update').onclick = async event => { event.preventDefault(); $('#config-dialog').close(); $('#update-dialog').showModal(); try { const value = await api('/api/update'); $('#update-status').textContent = value.update_available ? ui(`Version ${value.latest_version} ist verfügbar. Installiert: ${value.current_version}.`,`Version ${value.latest_version} is available. Installed: ${value.current_version}.`) : ui(`Installierte Version ${value.current_version} ist aktuell.`,`Installed version ${value.current_version} is up to date.`); $('#install-update').disabled = !value.update_available || value.state === 'running'; } catch (_) { $('#update-status').textContent = ui('Updateprüfung nicht möglich.','Could not check for updates.'); } };
...
$('#save-mqtt').onclick = async event => { event.preventDefault(); const body = {enabled:$('#mqtt-enabled').checked, host:$('#mqtt-host').value, port:+$('#mqtt-port').value, client_id:$('#mqtt-client-id').value, username:$('#mqtt-user').value, password:$('#mqtt-password').value, keepalive:+$('#mqtt-keepalive').value, qos:+$('#mqtt-qos').value, retain:$('#mqtt-retain').checked, tls_enabled:$('#mqtt-tls').checked, tls_ca_file:$('#mqtt-ca-file').value, discovery_prefix:$('#mqtt-discovery').value, base_topic:$('#mqtt-topic').value}; try { await api('/api/config/mqtt', json('PUT', body)); $('#mqtt-dialog').close(); toast(ui('MQTT-Einstellungen gespeichert.','MQTT settings saved.')); } catch (_) { toast(ui('MQTT-Einstellungen sind ungültig.','MQTT settings are invalid.'), true); } };
$('#install-update').onclick = async event => { event.preventDefault(); if (!confirm(ui('Update jetzt installieren? Die Messung wird kurz neu gestartet.','Install update now? Measurement will restart briefly.'))) return; try { await api('/api/update', {method:'POST', headers:{'X-NoiseMeter-Update':'confirm'}}); $('#install-update').disabled = true; $('#update-status').textContent = ui('Update angefordert. Installation startet im Hintergrund …','Update requested. Installation is starting in the background …'); } catch (_) { toast(ui('Update konnte nicht gestartet werden.','Update could not be started.'), true); } };
