...
function drawHistory(points) {
  ...
  const values = points.map(point => point.db), minimum = Math.floor(Math.min(...values) / 5) * 5 - 5, maximum = Math.ceil(Math.max(...values) / 5) * 5 + 5;
  ...
  context.strokeStyle = '#fff'; context.lineWidth = 2; context.beginPath(); points.forEach((point, index) => { const xx = left + index / Math.max(points.length - 1, 1) * chartWidth, yy = y(point.db); index ? context.lineTo(xx, yy) : context.moveTo(xx, yy); }); context.stroke();
...
function eventRows(events) {
  $('#events').innerHTML = events.length ? events.map(event => {
    ...
    return `<tr class="event-${severity}"><td>${dateFormat(event.occurred_at)}</td><td>${db(event.peak_db)}</td><td>${db(event.threshold_db)}</td><td>${event.period_name}</td><td><audio controls preload="none" src="/audio/${encodeURI(event.filename)}"></audio></td></tr>`;
  }).join('') : '<tr><td colspan="5">Keine Ereignisse in diesem Zeitraum.</td></tr>';
}
async function loadPeriodStats() {
  if (kind !== 'day') return;
  const result = await api(`/api/period-statistics?date=${localDay(selected)}`);
  $('#period-stats').innerHTML = result.items.map(item => `<article><div class="period-title"><strong>${item.name}</strong><small>${item.start}–${item.end} Uhr</small></div><dl><div><dt>Ereignisse</dt><dd>${item.event_count}</dd></div><div><dt>Maximum</dt><dd>${db(item.maximum_db)}</dd></div><div><dt>Durchschnitt</dt><dd>${db(item.average_db)}</dd></div><div><dt>Minimum</dt><dd>${db(item.minimum_db)}</dd></div></dl>${item.measurement_count ? '' : '<p class="no-data">Keine gültigen Messwerte</p>'}</article>`).join('');
}
async function loadBreakdown() {
  ...
  $('#breakdown').innerHTML = `<h2>${names[kind]}</h2><div class="table-scroll"><table><thead><tr><th>Zeitraum</th><th>Maximalpegel</th><th>Durchschnittspegel</th></tr></thead><tbody>${result.items.map(item => `<tr><td>${item.label}</td><td>${db(item.maximum_db)}</td><td>${db(item.average_db)}</td></tr>`).join('') || '<tr><td colspan="3">Keine gültigen Messwerte.</td></tr>'}</tbody></table></div>`;
}
async function loadEvents() {
  setTitle();
  try {
    const result = await api(`/api/events?kind=${kind}&date=${periodValue()}`);
    $('#count').textContent = result.summary.event_count; $('#peak').textContent = db(result.summary.peak_db); $('#average').textContent = db(result.summary.average_db); eventRows(result.events);
...
async function loadConfig() {
  ...
  $('#weighting').value = value.audio.weighting; $('#time-weighting').value = value.audio.time_weighting; $('#manual-calibration').value = value.audio.manual_calibration_db; $('#calibration-file').textContent = value.audio.calibration_file ? `Aktive Datei: ${value.audio.calibration_file}` : 'Keine Kalibrierdatei hinterlegt.';
  $('#microphone-name').value = value.audio.microphone_name || ''; $('#site-name').value = value.site_name; $('#site-location').value = site.location || ''; $('#site-orientation').value = site.orientation || ''; $('#site-target').value = site.target_object || ''; $('#site-ground').value = site.ground_distance || ''; $('#site-wall').value = site.wall_distance || '';
...
$('#save-site').onclick = async event => { event.preventDefault(); try { await api('/api/config/site', json('PUT', {site_name:$('#site-name').value, location:$('#site-location').value, orientation:$('#site-orientation').value, target_object:$('#site-target').value, ground_distance:$('#site-ground').value, wall_distance:$('#site-wall').value})); $('#site-dialog').close(); toast('Messstellendaten gespeichert.'); } catch (_) { toast('Messstellendaten sind ungültig.', true); } };
