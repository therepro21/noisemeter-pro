...
def create_report(path: Path, title: str, start: str, end: str, summary: dict, events: list[dict], site_name: str, site_data: dict | None = None, breakdown: list[dict] | None = None):
    ...
    if breakdown:
        rows2 = [["Zeitabschnitt", "Maximalpegel", "Durchschnittspegel"]] + [[x["label"], f"{x['maximum_db']:.1f} dB", f"{x['average_db']:.1f} dB"] for x in breakdown]
        bt = Table(rows2, repeatRows=1, colWidths=[6*cm,5*cm,5*cm]); bt.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#2a6f97")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("GRID",(0,0),(-1,-1),.25,colors.lightgrey),("PADDING",(0,0),(-1,-1),5)])); story += [Paragraph("Pegelauswertung", styles["Heading3"]), bt, Spacer(1,.5*cm)]
