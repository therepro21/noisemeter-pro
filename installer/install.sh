...
apt-get update
apt-get install -y python3-venv python3-pip portaudio19-dev ffmpeg
id -u noisemeter >/dev/null 2>&1 || useradd --system --home /var/lib/noisemeter --create-home --groups audio noisemeter
install -d -o noisemeter -g noisemeter /var/lib/noisemeter/audio /var/lib/noisemeter/reports /etc/noisemeter
